defmodule Mandel do
  ...
  def mandelbrot(width, height, x, y, k, depth) do
    ...
    Enum.chunk_every((height-1)..0, 100)
    |>Enum.map(fn chunk ->
          chunk = Enum.reverse(chunk)
          Task.async(fn ->
          rows(width, chunk, trans, depth, [])
        end)
      end)
      |> Enum.flat_map(&Task.await(&1, :infinity))
  end

  defp rows(_, [], _, _, acc), do: acc
  defp rows(width, [height | htail], trans, depth, acc) do

    row = row(width, height, trans, depth, [])
    rows(width, htail, trans, depth, [row | acc])
